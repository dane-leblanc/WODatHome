from models import Equipment, Category, Exercise, ExerciseImage
...
def get_exercises(equip_list, name_input, category_input):
    ...
    # Filter by category (if user did not select a category,
    # do not include a category on the exercise filter)
    if category_input != '<none>':
        cat = Category.query.filter_by(name=category_input).one()

        all_exercises = Exercise.query.filter(
            Exercise.name.ilike(f'%{name_input}%'),
            Exercise.category_id == cat.id).all()

    else:
        all_exercises = Exercise.query.filter(
            Exercise.name.ilike(f'%{name_input}%')).all()

    exercises = []
    # Check the equipment needed for each exercise against
    # the equipment the user marked as 'available'.
    # Remove any exercises that require equipment beyond what's available.
    # Also remove exercises with no equipment listed as these seem to either
    # have other items of info missing or require very specific
    # gym (machine) equipment.
    for exercise in all_exercises:
        if set(exercise.equipment).issubset(
                set(equip_list)) is True and exercise.equipment != []:
